...
const CartProvider = ({ children }) => {
    ...
    try {
      const fetchedProducts = await Promise.all(
        products.map(async (product) => {
          const productData = await fetchGraphQLData(QUERY_GET_PRODUCT, {
            product_id: product.product_id,
          });
          if (productData?.products?.length > 0) {
            const productDetails = productData.products[0];
            const imageResponse = await fetchGraphQLData(
              QUERY_GET_ATTACHMENT_BY_ID,
              {
                attachmentByIdId: productDetails.display_image,
              }
            );
            productDetails.display_image_url =
              imageResponse?.attachmentById?.data?.url ||
  ...
  const fetchCartData = async () => {
    try {
      ...
      if (!db_user_id) {
        console.error("User ID is not available. Unable to fetch cart data.");
        setCartLoading(false);
        return;
      }

      // Pass the user ID to the GraphQL query
      ...
      if (response?.cart) {
        ...
        // Fetch detailed product data and update the cart
        await fetchProducts(cartData.products);

        // Update the order summary
        await setOrderSummary();
      } else {
        console.error("Cart data is unavailable in the response.");
      }
    } catch (error) {
  ...
  const reduceQtyFromLocalStorage = (product_id, quantity) => {
    try {
      ...
      localStorage.setItem("cart", JSON.stringify(currentCart));
      console.log(
        "Updated cart in localStorage after quantity removal:",
        currentCart
      );
    } catch (error) {
      console.error("Error handling remove from cart:", error);
  ...
  const deleteItemFromLocalStorage = (product_id) => {
    try {
      ...
      localStorage.setItem("cart", JSON.stringify(currentCart));
      console.log(
        "Updated cart in localStorage after item removal:",
        currentCart
      );
    } catch (error) {
      console.error("Error handling delete item from cart:", error);
