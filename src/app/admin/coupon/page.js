...
export default function CouponPage() {
  ...
  const createCoupon = async () => {
    ...
    try {
      ...
      newCoupon.applicable_products = selectedProducts.map(
        (product) => product.value
      );
      newCoupon.applicable_categories = selectedCategories.map(
        (category) => category.value
      );
      newCoupon.applicable_subcategories = selectedSubcategories.map(
  ...
  const updateCoupon = async () => {
    ...
    const variables = {
      ...
      applicable_categories: selectedCategories.map(
        (category) => category.value
      ),
      applicable_subcategories: selectedSubcategories.map(
        (subCategory) => subCategory.value
      ),
    ...
    try {
      ...
      if (!data.error) {
        ...
        updateCoupon.valid_from = convertToHTMLDate(
          new Date(updatedCoupon.valid_from * 1)
        );
        updateCoupon.valid_until = convertToHTMLDate(
          new Date(updatedCoupon.valid_until * 1)
        );
  ...
  const validateForm = (formData) => {
    ...
    if (!selectedSubcategories.length) {
      errors.applicable_subcategories =
  ...
  const toggleModal = async (coupon = null) => {
    ...
    if (coupon) {
      ...
      const selectedCategories = coupon.applicable_categories.map(
        (category) => ({
          value: category.id,
          label: category.name,
        })
      ...
      const selectedSubcategories = coupon.applicable_subcategories.map(
        (subCategory) => ({
              ...
              <form className="space-y-4" onSubmit={handleCreateOrUpdate}>
                ...
                <div>
                  ...
                  <input
                    ...
                    value={
                      editingCoupon ? editingCoupon.code : newCoupon.code || ""
                    }
                ...
                <div>
                  ...
                  <select
                    ...
                    className="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400"
                    value={
                      editingCoupon ? editingCoupon.type : newCoupon.type || ""
                    }
                    onChange={handleInputChange}
                  >
                ...
                {/* Applicable Products */}
                <div>
                  <label className='className="block text-sm font-medium mb-2 dark:text-white"'>
                    Applicable Products
                  </label>
                  <Select
                    isMulti
                    ...
                    onChange={(selectedProducts) =>
                      handleSelect(selectedProducts, "product")
                    }
                    className="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400"
                    classNamePrefix="select"
                    placeholder="Select applicable products"
                  />
                  {formErrors.applicable_products && (
                    <p className="text-red-500 text-sm">
                      {formErrors.applicable_products}
                    </p>
                  )}
                ...
                <div>
                  ...
                  <Select
                    ...
                    onChange={(selectedCategories) =>
                      handleSelect(selectedCategories, "category")
                    }
                    className="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400"
                    classNamePrefix="select"
                    placeholder="Select applicable categories"
                  />
                  {formErrors.applicable_categories && (
                    <p className="text-red-500 text-sm">
                      {formErrors.applicable_categories}
                    </p>
                  )}
                ...
                <div>
                  ...
                  <Select
                    ...
                    onChange={(selectedSubcategories) =>
                      handleSelect(selectedSubcategories, "subcategory")
                    }
                    className="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400"
                    classNamePrefix="select"
                    placeholder="Select applicable subcategories"
                  />
                  {formErrors.applicable_subcategories && (
                    <p className="text-red-500 text-sm">
                      {formErrors.applicable_subcategories}
                    </p>
                  )}
                ...
                <div>
                  <label
                    htmlFor="coupon-max-use-per-user"
                    className="block text-sm font-medium mb-2 dark:text-white"
                  >
                    Max Use Per User
                  </label>
                  <input
                    ...
                    value={
                      editingCoupon
                        ? editingCoupon.max_use_per_user
                        : newCoupon.max_use_per_user || ""
                    }
                    onChange={handleInputChange}
                  />
                  {formErrors.max_use_per_user && (
                    <p className="text-red-500 text-sm">
                      {formErrors.max_use_per_user}
                    </p>
                  )}
